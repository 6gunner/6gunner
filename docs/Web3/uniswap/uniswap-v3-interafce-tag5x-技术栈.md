...
从这个版本开始，uniswap interface改成了 mono-repo的形式；
...
创建apps/web/src/utils/__generated__目录

yarn run lfg

报错： bash: ../../scripts/downloadEnvLocal.sh: No such file or directory





在root下创建`.env.defaults`,  copy the configuration in config/jest-presets/jest/globals.js to the env file. 然后格式化一下
...
路由文件定义位置：`src/pages/RouteDefinitions.tsx`